...
#             print final_straight
            ...
            final_df=final_df.sort_values(by=['ProcessCategory','Mapping Reference Number','Test case Category','Query Type'])
...
#                                print row[row_bdf['TargetValue'].strip().upper()]
                               final_vardf_base_straight = final_vardf_base_straight.append(pd.DataFrame([[row['TRACEBILITY ID'],row_bdf['QuerySequenceNo'],row_bdf['Category'],row_bdf['Query Type'],row_bdf['AttributeType'],row_bdf['ColumnHeader'],row_bdf['AttributeName'],row[row_bdf['TargetValue'].strip().upper()]]],columns=['Tracebility ID','QuerySequenceNo','Category','Query Type','AttributeType','ColumnHeader','AttributeName','Value']), ignore_index=True)
        #                 if (str(row_bdf['AttributeName']).strip()=='sourceColumnName'):  
        #                        row_bdf['AttributeName'] = row['Source Field(s)/ Column(s)'] 
...
#                                print row[row_cdf['TargetValue'].strip().upper()]
                               final_vardf_comapre_straight = final_vardf_comapre_straight.append(pd.DataFrame([[row['TRACEBILITY ID'],row_cdf['QuerySequenceNo'],row_cdf['Category'],row_cdf['Query Type'],row_cdf['AttributeType'],row_cdf['ColumnHeader'],row_cdf['AttributeName'],row[row_cdf['TargetValue'].strip().upper()]]],columns=['Tracebility ID','QuerySequenceNo','Category','Query Type','AttributeType','ColumnHeader','AttributeName','Value']), ignore_index=True)
...
#                         print row_bdf['AttributeName']
                    ...
                    for index,row_cdf in vardf_comparedeafult.iterrows():
                        ...
                        if (row_cdf['AttributeName']=='tableName'):
                               #row_cdf['AttributeName'] = row['Target Physical Column Name']
                               condition_tablename=row[row_cdf['TargetValue'].strip().upper()]
                        ...
                        if (str(row_cdf['AttributeName']).strip()=='condition1'):  
                               conditionVal = condition_tablename \
                                                          + '.' + row[row_cdf['TargetValue'].strip().upper()] + '='  + self.format_function(str(row['DEFAULT VALUE']))
...
#             dfrdmcmptemplate=dfrdmcmptemplate[dfrdmcmptemplate.columns[-int(rdmcmpiter):]]
            ...
            for index_map,row_map in maindf.iterrows():
                ...
                for index_tmpl,row_base in vardf_baserdm.iterrows():
                    ...
                    if str(row_base['AttributeName']).strip().upper() == 'SOURCECOLUMNNAME':
                        varstring= source_tab_nm + '.' + row_map[row_base['TargetValue'].strip().upper()]
                        ...
                        colname=str((re.findall(r"\w+[.]\w+",(re.findall(r"step[0-9]+[:](.+)referring",str(row_map[row_base['TargetValue'].strip().upper()]).strip(),re.IGNORECASE))[0],re.IGNORECASE))[0])                   
    ...
    #                   dfrdmbasetmpl=dfrdmbasetmpl.append(pd.DataFrame([[row_map['Tracebility ID'],row_tmpl['Category'],row_tmpl['Query Type'],row_tmpl['ColumnHeader'],row_tmpl['AttributeName'],row_map[row_tmpl['TargetValue']]]],columns=['Tracebility ID','Category','Query Type','ColumnHeader','AttributeName','Value']))
                        ...
                        dfrdm_basevar=dfrdm_basevar.append(pd.DataFrame([[row_map['TRACEBILITY ID'],row_base['QuerySequenceNo'],row_base['Category'],row_base['Query Type'],row_base['AttributeType'],row_base['ColumnHeader'],row_base['AttributeName'],row_map[row_base['TargetValue'].strip().upper()]]],columns=['Tracebility ID','QuerySequenceNo','Category','Query Type','AttributeType','ColumnHeader','AttributeName','Value']), ignore_index=True)
                    elif str(row_base['AttributeName']).strip().upper() == 'RDMTABLE':
                        colnm=str((re.findall(r"\w+[.]\w+",(re.findall(r"step[0-9]+[:](.+)referring",str(row_map[row_base['TargetValue'].strip().upper()]).strip(),re.IGNORECASE))[0],re.IGNORECASE))[0])
...
#                 print dfrdm_basevar
                ...
                for index,row_cdf in vardf_comparerdm.iterrows():
                    ...
                    if str(row_cdf['AttributeName']).strip().upper() == 'COLUMNNAME':
                        targetcolname= target_tab_nm + '.' + row_map[row_cdf['TargetValue'].strip().upper()]
                        ...
                        dfrdm_cmpvar=dfrdm_cmpvar.append(pd.DataFrame([[row_map['TRACEBILITY ID'],row_cdf['QuerySequenceNo'],row_cdf['Category'],row_cdf['Query Type'],row_cdf['AttributeType'],row_cdf['ColumnHeader'],row_cdf['AttributeName'],row_map[row_cdf['TargetValue'].strip().upper()]]],columns=['Tracebility ID','QuerySequenceNo','Category','Query Type','AttributeType','ColumnHeader','AttributeName','Value']), ignore_index=True)
#                 print dfrdm_cmpvar
        ...
        try:
            ...
            for src_tab_name in maindf['SOURCE TABLE(S)'].unique():
                ...
                for index,row in maindf.iterrows():
                    ...
                    if ((str(testCaseGenerationConfig.sourceSystemName).strip().upper() == 'SQLSERVER') and (src_tab_name.strip().upper() not in ['NA','N/A']) and (str(row['SOURCE TABLE(S)']).strip().upper()==src_tab_name.strip().upper()) and (row['OFFSET COLUMN']=='Y')):
                        src_generic_df= src_generic_df.append(pd.DataFrame([[row['SOURCE TABLE(S)'],row['SOURCEOFFSETCOLUMN'],row['TARGET PHYSICAL TABLE NAME'],row['TARGETOFFSETCOLUMN']]],columns=['Source Table Name','SourceOffsetcolumn','Target Physical Table Name','TargetOffsetColumn']))
                        break
                    if ((str(testCaseGenerationConfig.sourceSystemName).strip().upper() == 'HIVE') and (src_tab_name.strip().upper() not in ['NA','N/A']) and (str(row['SOURCE TABLE(S)']).strip().upper()==src_tab_name.strip().upper()) and (row['OFFSET COLUMN']=='')):
                        src_generic_df= src_generic_df.append(pd.DataFrame([[src_tab_name,'',row['TARGET PHYSICAL TABLE NAME'],'']],columns=['Source Table Name','SourceOffsetcolumn','Target Physical Table Name','TargetOffsetColumn']))
                        break
#             print src_generic_df
...
#                 source_tab_nm=row_map['Source table(s)']
                for index_tmpl,row_base in vardf_basetransfm.iterrows():
                    ...
                    if ((str(row_base['TargetSource']).strip().upper() == 'MAPPING') and (str(row_base['TargetValue']).strip().upper() not in  ['TRANSFORMATION RULE','JOINING CONDITION'])):
                        colval=row_map[row_base['TargetValue'].strip().upper()]
                    ...
                    if ((str(row_base['TargetSource']).strip().upper() == 'MAPPING') and (str(row_base['TargetValue']).strip().upper() =='TRANSFORMATION RULE')):
                        if re.search(r"(^CASE WHEN)(.*)(?=THEN)(.*)(?=ELSE)(.*)(?=END)",str(row_map[row_base['TargetValue'].strip().upper()]).strip(),re.IGNORECASE)<>None:
                            #Between when...then
                            src_cond_list=re.findall(r"(?<=WHEN)(.*?)(?=THEN)",str(row_map[row_base['TargetValue'].strip().upper()]).strip(), re.IGNORECASE)
...
#                 print dftransfm_basevar
                for index_tmpl,row_cmp in vardf_comparetransfm.iterrows():
                    ...
                    if ((str(row_cmp['TargetSource']).strip().upper() == 'MAPPING') and (str(row_cmp['TargetValue']).strip().upper() not in  ['TRANSFORMATION RULE','JOINING CONDITION'])):
                        colval=row_map[row_cmp['TargetValue'].strip().upper()]
                    ...
                    if ((str(row_cmp['TargetSource']).strip().upper() == 'MAPPING') and (str(row_cmp['TargetValue']).strip().upper() == 'TRANSFORMATION RULE')):
                        transform_text=str(row_map[row_cmp['TargetValue'].strip().upper()]).strip().upper()
